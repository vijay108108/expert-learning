  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          { label: "Total Revenue",    value: `₹${totalRevenue.toLocaleString("en-IN")}`,  color: "text-[#34D399]" },
          { label: "Total Payments",   value: filtered.length,                               color: "text-[#818CF8]" },
          { label: "Avg. Order Value", value: filtered.length ? `₹${Math.round(totalRevenue / filtered.length).toLocaleString("en-IN")}` : "—", color: "text-[#60A5FA]" },
        ].map((s) => (
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative min-w-[200px] flex-1">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Name, course or payment ID…"
            ...
            {loading ? (
              [...Array(6)].map((_, i) => (
                <tr key={i} className="border-b border-white/6">
                  {[...Array(6)].map((__, j) => <td key={j} className="px-4 py-3"><div className="h-3.5 animate-pulse rounded bg-white/8 w-3/4" /></td>)}
                </tr>
              ))
            ) : filtered.length === 0 ? (
              <tr><td colSpan={6} className="px-4 py-8 text-center text-[#334155]">No payments found.</td></tr>
            ) : filtered.map((r) => (
              <tr key={r.id} className="border-b border-white/6 bg-[#0D1117] transition hover:bg-white/3">
                ...
                <td className="px-4 py-3 font-semibold text-[#34D399]">
                  {r.amountPaid ? `₹${r.amountPaid.toLocaleString("en-IN")}` : "—"}
                </td>
                ...
                <td className="px-4 py-3 text-[#475569]">
                  {r.enrolledAt ? new Date(r.enrolledAt).toLocaleDateString("en-IN") : "—"}
                </td>
              </tr>
            ))}
          </tbody>
