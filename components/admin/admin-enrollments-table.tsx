    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative min-w-[220px] flex-1">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name, course or phone…"
        ...
        <table className="w-full text-[13px]">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-8 text-center text-[#334155]">No enrollments found.</td></tr>
            ) : filtered.map((r) => (
              <tr key={r.id} className="border-b border-white/6 bg-[#0D1117] transition hover:bg-white/3">
                ...
                <td className="px-4 py-3 text-[#34D399]">
                  {r.amountPaid ? `₹${r.amountPaid.toLocaleString("en-IN")}` : "Free"}
                </td>
                <td className="px-4 py-3">
